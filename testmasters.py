...
# 3. Savollar bazasi (Matematika, Fizika, Informatika)
questions = {
    "Matematika": [
...
subject = st.selectbox("Fan tanlang:", list(questions.keys()))

# Siz yuborgan jadval havolasi (Sizning ID raqamingiz)
SHEET_URL = "https://docs.google.com/spreadsheets/d/1s_Q6s_To2pI63gqqXWmGfkN_H2yIO42KIBA8G5b0B4U/edit#gid=1015362765"
if name:
    st.divider()
    score = 0
    ...
    if st.button("Natijani bazaga yuborish"):
        try:
            ...
            # Google Sheets'dan joriy ma'lumotni o'qish (ttl=0 keshlashni o'chiradi)
            ...
            st.success(f"Muvaffaqiyatli! {name}, natijangiz saqlandi: {score}/10")
            st.balloons()
            
        except Exception as e:
            st.error(f"Xatolik yuz berdi: {e}")
            st.info("Eslatma: Jadvalda 'Ism', 'Fan', 'Ball' ustunlari borligini tekshiring.")
else:
    st.warning("Iltimos, ismingizni yozib testni boshlang.")
